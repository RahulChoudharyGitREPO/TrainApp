...
try {
  ...
  console.log('⚠️  SendGrid not installed, using nodemailer only');
}
...
export const sendOTPEmail = async (email, otp, name) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `"Tixigo Train Booking" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Your OTP for Train Booking System',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    console.log('⚠️  Email send failed, OTP saved to DB for', email);
